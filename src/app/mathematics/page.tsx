...
import MathsQuestionDetails from "@/components/MathsQuestionDetails";
...
type PaperPreviewProps = {
  paper: MathsPaperQuestion[];
  totalMarks: number;
  formattedQuestions: Question[];
  removeQuestion: (id: string) => void;
  regenerate: () => void;
  replaceQuestion: (index: number, replacement: Question) => void;
  moveQuestion: (index: number, direction: number) => void;
  reorderQuestion: (fromIndex: number, toIndex: number) => void;
  updateQuestionParts: (index: number, selectedParts: string[] | undefined, marks: number) => void;
};
...
function PaperPreview({paper,totalMarks,formattedQuestions,removeQuestion,regenerate,replaceQuestion,moveQuestion,reorderQuestion,updateQuestionParts}: PaperPreviewProps) {
  ...
  async function downloadWord() {
    setExporting(true);
    try { await exportMathsPaperToWord(paper); }
    catch (error) { window.alert(error instanceof Error ? error.message : "Could not build the Word paper."); }
    finally { setExporting(false); }
  }

  async function downloadMarkScheme() {
    if (hasPartialQuestions) {
      window.alert("Partial Maths questions are supported in the question paper, but cropped sub-question mark schemes are not connected yet. Use the full-question mark scheme for now.");
      return;
    }
    ...
    setExportingMarkScheme(true);
    try { await exportMarkSchemeToWord(paper); }
    catch (error) { window.alert(error instanceof Error ? error.message : "Could not build the mark scheme."); }
    finally { setExportingMarkScheme(false); }
  }
  ...
  return <div className="panel preview">
    ...
    {!paper.length ? <div className="empty"><div>✦</div><h3>Your custom paper will appear here</h3><p>Pick topics and settings, then generate a balanced selection.</p></div> : <>
      <div className="editorHint"><span>✦</span><div><b>Edit before you export</b><small>Drag and drop questions, swap them, or choose exactly which sub-questions to keep.</small></div></div>
      <div className="questionList">{paper.map((q,index) => <article className="question" key={`${q.id}-${index}`} draggable onDragStart={e => {setDraggedIndex(index);e.dataTransfer.effectAllowed="move";}} onDragOver={e => {e.preventDefault();e.dataTransfer.dropEffect="move";}} onDrop={e => {e.preventDefault();if(draggedIndex!==null)reorderQuestion(draggedIndex,index);setDraggedIndex(null);}} onDragEnd={() => setDraggedIndex(null)} style={{cursor:"grab",opacity:draggedIndex===index?.55:1,transition:"opacity .15s ease, transform .15s ease"}}>
        <div className="qNumber" title="Drag to reorder">{index+1}<div style={{fontSize:10,lineHeight:1,opacity:.55,marginTop:4}}>⋮⋮</div></div>
        <div className="qBody"><div className="qMeta">{q.session} {q.year} · Paper {q.paper} · Original Q{q.questionNumber} · {q.difficulty}</div><h3>{q.summary}</h3><div className="tags">{q.topics.map(t => <span key={t}>{t}</span>)}</div><MathsPartPicker question={q} onChange={(parts,marks) => updateQuestionParts(index,parts,marks)}/><div className="questionTools"><button onClick={() => moveQuestion(index,-1)} disabled={index===0}>↑ Up</button><button onClick={() => moveQuestion(index,1)} disabled={index===paper.length-1}>↓ Down</button><button onClick={() => setSwapIndex(index)}>↻ Swap</button><button onClick={() => removeQuestion(q.id)}>Remove</button></div></div>
...
function SwapQuestionModal({current,paper,questions,onClose,onChoose}:{current:MathsPaperQuestion;paper:MathsPaperQuestion[];questions:Question[];onClose:()=>void;onChoose:(q:Question)=>void}) {
  ...
  return <div onMouseDown={e => {if(e.target===e.currentTarget)onClose();}} style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,.78)",backdropFilter:"blur(7px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
    <div style={{width:"min(800px,96vw)",maxHeight:"88vh",display:"flex",flexDirection:"column",background:"#101010",border:"1px solid rgba(201,162,39,.38)",borderRadius:18,boxShadow:"0 28px 80px rgba(0,0,0,.62)",overflow:"hidden"}}>
      <div style={{padding:"18px 20px 14px",borderBottom:"1px solid rgba(255,255,255,.08)",display:"flex",justifyContent:"space-between",gap:16,alignItems:"flex-start"}}>
        <div><div style={{fontSize:11,fontWeight:900,letterSpacing:".12em",color:"#d2ad36",marginBottom:5}}>SWAP QUESTION</div><div style={{fontSize:18,fontWeight:850}}>Choose the replacement yourself</div><div style={{fontSize:12,color:"#989898",marginTop:5}}>Click any question to open its full wording and diagram before you swap it in.</div></div>
      ...
      <div style={{padding:"10px 20px 18px",overflowY:"auto"}}>
        ...
        {candidates.length===0 ? <div style={{padding:"28px 10px",textAlign:"center",color:"#888"}}>No other questions match this topic and search.</div> : candidates.map(q => {
          ...
          return <div key={q.id} style={{background:"#141414",border:`1px solid ${expanded?"rgba(201,162,39,.36)":"rgba(255,255,255,.08)"}`,borderRadius:12,padding:"13px 14px",marginBottom:8}}>
            <div role="button" tabIndex={0} onClick={() => setExpandedId(expanded ? null : q.id)} onKeyDown={e => {if(e.key==="Enter"||e.key===" ")setExpandedId(expanded ? null : q.id);}} style={{display:"grid",gridTemplateColumns:"1fr auto",gap:14,alignItems:"center",cursor:"pointer"}}>
            ...
            {expanded && <>
              <MathsQuestionDetails question={q}/>
