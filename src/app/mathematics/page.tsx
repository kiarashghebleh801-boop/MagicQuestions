...
export default function MathematicsPage() {
  ...
  function swapQuestion(index: number) {
    ...
    setPaper(current => current.map((q,i)=>i===index?wrapQuestion(candidates[0]):q));
  }
  ...
  return <main>
    ...
    {mode==="generate" ? <section className="builder">
      ...
      <PaperPreview paper={paper} totalMarks={totalMarks} removeQuestion={removeQuestion} regenerate={generate} swapQuestion={swapQuestion} moveQuestion={moveQuestion} reorderQuestion={reorderQuestion} updateQuestionParts={updateQuestionParts}/>
...
type PaperPreviewProps={paper:MathsPaperQuestion[];totalMarks:number;removeQuestion:(id:string)=>void;regenerate:()=>void;swapQuestion:(index:number)=>void;moveQuestion:(index:number,direction:number)=>void;reorderQuestion:(fromIndex:number,toIndex:number)=>void;updateQuestionParts:(index:number,selectedParts:string[]|undefined,marks:number)=>void};
function PaperPreview({paper,totalMarks,removeQuestion,regenerate,swapQuestion,moveQuestion,reorderQuestion,updateQuestionParts}:PaperPreviewProps){
