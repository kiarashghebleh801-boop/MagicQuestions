...
import ChemistryReviewTracker from "@/components/ChemistryReviewTracker";
...
import ChemistryPartPicker from "@/components/ChemistryPartPicker";
...
export default function ChemistryPage() {
  ...
  const allSpecTopics = useMemo(() => chemistrySections.flatMap(section => section.subtopics.map(sub => ({ tag: `${section.number}${sub.code}`, sectionNumber: section.number, sectionTitle: section.title, code: sub.code, title: sub.title }))), []);
  ...
  const filteredBank = useMemo(() => { const needle = query.trim().toLowerCase(); return topicSpecificQuestions.filter(q => !needle || `${q.summary} ${q.session} ${q.year} ${q.paper} q${q.questionNumber} ${q.specTags.join(" ")}`.toLowerCase().includes(needle)); }, [query, topicSpecificQuestions]);

  if (!ready) return <main className="authPage"><div className="authLogo"><span>✦</span> MagicQuestions Chemistry</div></main>;

  function toggleTopic(tag: string) { setSelectedTopics(current => current.includes(tag) ? current.filter(x=>x!==tag) : [...current, tag]); }
  function toggleCompleted(tag: string) { setCompletedTopics(current => current.includes(tag) ? current.filter(x => x !== tag) : [...current, tag]); }
  function generate() { const shuffled = [...topicSpecificQuestions].sort(() => Math.random() - .5); setPaper(shuffled.slice(0, Math.min(count, shuffled.length))); }
  function addQuestion(q: ChemistryQuestion) { const specialised = specialiseChemistryQuestion(q, selectedTopics); if (!specialised) { window.alert("That question contains material outside the selected specification topic(s)."); return; } setPaper(current => current.some(x=>x.id===specialised.id) ? current : [...current,specialised]); }
  function removeQuestion(id: string) { setPaper(current => current.filter(q=>q.id!==id)); }
  function moveQuestion(index:number,direction:number) { setPaper(current => { const next=index+direction; if(next<0||next>=current.length) return current; const copy=[...current]; [copy[index],copy[next]]=[copy[next],copy[index]]; return copy; }); }
  function replaceQuestion(index: number, replacement: ChemistryQuestionSelection) {
    setPaper(current => current.map((q, i) => i === index ? replacement : q));
  }
  async function downloadWord() { if(!paper.length) return; setExporting(true); try { await exportPaperToWord(paper.map(asExportQuestion), { subject: "Chemistry" }); } catch (error) { window.alert(error instanceof Error ? error.message : "Could not build the Chemistry Word paper."); } finally { setExporting(false); } }
  ...
  return <main>
    ...
    <section className="hero"><p className="eyebrow">EDEXCEL INTERNATIONAL GCSE CHEMISTRY 4CH1</p><h1>Your topics. Your Chemistry paper.<br/><span>Generated in seconds.</span></h1><p className="subtitle">Choose exact specification sections. MagicQuestions now keeps only the parts of each question that test the topic(s) you selected, removes unrelated parts, and renumbers the remaining parts from (a).</p></section>
    ...
    {mode==="generate" ? <section className="builder">
      <div className="panel controls">
        ...
        <div className="divider"/><div className="step"><span>2</span><div><b>Paper settings</b><small>Choose how many questions</small></div></div>
      ...
      <div className="panel preview">
        ...
        {!paper.length?<div className="empty"><div>⚗</div><h3>Your Chemistry paper will appear here</h3><p>Pick specification topics, choose the number of questions, then generate.</p></div>:<><div className="editorHint"><span>✦</span><div><b>Topic-specific extraction is on</b><small>Unrelated subparts are removed. If original parts (c) and (d) are kept, they export as (a) and (b).</small></div></div><div className="questionList">{paper.map((q,index)=><article className="question" key={`${q.id}-${index}`}><div className="qNumber">{index+1}</div><div className="qBody"><div className="qMeta">{q.session} {q.year} · Paper {q.paper} · Original Q{q.questionNumber}{q.selectedParts?.length?` · Parts ${q.selectedParts.map(p=>`(${p})`).join(", ")}`:""}</div><h3>{q.summary}</h3><div className="tags">{q.specTags.map(tag=><span key={tag}>Spec {tag[0]}({tag.slice(1)})</span>)}</div><ChemistryPartPicker question={q} onChange={updated=>setPaper(current=>current.map((item,i)=>i===index?updated:item))}/><button type="button" onClick={()=>setExpandedQuestionId(current=>current===q.id?null:q.id)} aria-expanded={expandedQuestionId===q.id} style={{marginTop:10,border:"1px solid rgba(201,162,39,.38)",background:"rgba(201,162,39,.08)",color:"#e4c65f",borderRadius:8,padding:"7px 10px",cursor:"pointer",fontWeight:700}}>{expandedQuestionId===q.id?"Hide full question ↑":"View full question ↓"}</button>{expandedQuestionId===q.id&&<><div className="qMeta" style={{marginTop:8}}>Original source question{q.selectedParts?.length?` · Your export keeps only ${q.selectedParts.map(p=>`(${p})`).join(", ")}`:""}</div><ScienceQuestionDetails question={asExportQuestion(q)}/></>}<div className="questionTools"><button onClick={()=>moveQuestion(index,-1)} disabled={index===0}>↑ Up</button><button onClick={()=>moveQuestion(index,1)} disabled={index===paper.length-1}>↓ Down</button><button onClick={()=>setSwapIndex(index)}>↻ Swap</button><button onClick={()=>removeQuestion(q.id)}>Remove</button></div></div><div className="marks">{q.marks}<small>marks</small></div></article>)}</div><div className="paperActions"><button onClick={generate}>↻ Regenerate all</button><button className="word" onClick={downloadWord} disabled={exporting}>{exporting?"Building Word…":"Download Word"}</button><button onClick={()=>setReviewOpen(true)}>✓ Review paper</button></div>{reviewOpen&&<PaperReview paper={paper.map(asExportQuestion)} title="Chemistry paper review" onClose={()=>setReviewOpen(false)}/>}</>}
        {swapIndex!==null&&paper[swapIndex]&&<ScienceSwapModal subject="Chemistry" current={asExportQuestion(paper[swapIndex])} paper={paper.map(asExportQuestion)} questions={topicSpecificQuestions.map(asExportQuestion)} topicLabel={tag=>`Spec ${tag[0]}(${tag.slice(1)})`} onClose={()=>setSwapIndex(null)} onChoose={q=>{const chosen=topicSpecificQuestions.find(candidate=>candidate.id===q.id);if(chosen)replaceQuestion(swapIndex,chosen);setSwapIndex(null);}}/>}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"220px 1fr",gap:28,alignItems:"center",marginBottom:28}}>
        <div style={{width:180,height:180,borderRadius:"50%",display:"grid",placeItems:"center",background:`conic-gradient(var(--accent, #6c4cff) ${trackerPercent}%, rgba(127,127,127,.18) 0)`,padding:14,margin:"0 auto"}}><div style={{width:"100%",height:"100%",borderRadius:"50%",background:"var(--panel, #fff)",display:"grid",placeItems:"center",textAlign:"center"}}><div><strong style={{fontSize:38}}>{trackerPercent}%</strong><div className="qMeta">complete</div></div></div></div>
      ...
      <ChemistryReviewTracker/>
      <div style={{display:"flex",flexDirection:"column",gap:18}}>{chemistrySections.map(section => { const sectionTags = section.subtopics.map(sub => `${section.number}${sub.code}`); const sectionDone = sectionTags.filter(tag => completedTopics.includes(tag)).length; const sectionPercent = Math.round((sectionDone / sectionTags.length) * 100); return <div key={section.number} style={{border:"1px solid var(--border, rgba(127,127,127,.2))",borderRadius:16,padding:18}}><div style={{display:"flex",justifyContent:"space-between",gap:16,alignItems:"center",marginBottom:12}}><div><div className="qMeta">SECTION {section.number}</div><h3 style={{margin:"2px 0"}}>{section.title}</h3></div><b>{sectionPercent}%</b></div><div style={{height:8,borderRadius:999,background:"rgba(127,127,127,.16)",overflow:"hidden",marginBottom:14}}><div style={{height:"100%",width:`${sectionPercent}%`,background:"var(--accent, #6c4cff)",borderRadius:999}}/></div><div style={{display:"grid",gap:9}}>{section.subtopics.map(sub => { const tag = `${section.number}${sub.code}`; const done = completedTopics.includes(tag); return <label key={tag} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 12px",borderRadius:12,cursor:"pointer",background:done?"rgba(108,76,255,.10)":"rgba(127,127,127,.06)"}}><input type="checkbox" checked={done} onChange={()=>toggleCompleted(tag)} style={{width:18,height:18}}/><span style={{textDecoration:done?"line-through":"none",opacity:done?.7:1}}><b>{section.number}({sub.code})</b> {sub.title}</span></label> })}</div></div> })}</div>
    </section>}
  </main>;
}
