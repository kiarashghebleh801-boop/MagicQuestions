...
export default function ChemistryPage() {
  ...
  function swapQuestion(index:number) { const old=paper[index]; if(!old) return; const used=new Set(paper.map(q=>q.id)); const candidates=topicSpecificQuestions.filter(q=>!used.has(q.id)&&q.specTags.some(tag=>old.specTags.includes(tag))); if(!candidates.length){window.alert("No other topic-specific Chemistry question is available yet.");return;} setPaper(current=>current.map((q,i)=>i===index?candidates[0]:q)); }
