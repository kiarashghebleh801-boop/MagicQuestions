...
export default function Home() {
  ...
  function generate() { const ranked = generateQuestions(selected, questions.length, difficulty).filter(hasFormattedSource); setPaper(ranked.slice(0, count)); }
  ...
  function swapQuestion(index: number) {
    ...
    if (!candidates.length) { window.alert("No other formatted question for the same topic is available yet."); return; }
    candidates.sort((a,b)=>{
      const overlap=(q:Question)=>q.topics.filter(t=>old.topics.includes(t)).length;
      const score=(q:Question)=>overlap(q)*10+(q.difficulty===old.difficulty?3:0)-Math.abs(q.marks-old.marks);
...
function PaperPreview({paper,totalMarks,removeQuestion,regenerate,swapQuestion,moveQuestion}:PaperPreviewProps){
  ...
  const markSchemeReady=paper.length>0&&paper.every(hasMarkSchemeSource);
  ...
  async function downloadMarkScheme(){setExportingMarkScheme(true);try{await exportMarkSchemeToWord(paper);}catch(error){window.alert(error instanceof Error?error.message:"Could not build the mark scheme.");}finally{setExportingMarkScheme(false);}}
